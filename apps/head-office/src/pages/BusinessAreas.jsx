...
export default function BusinessAreas({ token }) {
  ...
  const loadAreas = async () => {
    try {
      setLoading(true);
      setError("");
      const data = await getBusinessAreas(token);
  ...
  const loadDashboard = async () => {
    try {
      ...
      setDashboardError("");
      const data = await getBusinessAreasDashboard(token, { timePreset });
  ...
  const handleSaveArea = async () => {
    ...
    try {
      ...
      if (editingId) {
        await updateBusinessArea(token, editingId, payload);
      } else {
        await createBusinessArea(token, payload);
      }
  ...
  return (
    <section className="page-container business-area-page">
      <div className="page-header">
        <div>
          <h1>Khu vực kinh doanh</h1>
          <p className="stat-text">Quản lý danh sách khu vực kinh doanh</p>
        </div>
      ...
      <div className="business-area-dashboard-wrap">
        ...
        <div className="panel business-area-chart-card">
